...
Эта запись заменяет прежнее описание ширины и загрузки ниже. Текстовая кнопка имеет ширину по содержимому, минимум 96 px и максимум по контейнеру; автоматически в grid/flex не растягивается. При одной иконке симметричный пустой слот сохраняет подпись по центру всей кнопки.

При loading вместо текста и иконок показывается вращающийся спиннер. Скрытый слой исходного содержимого сохраняет ширину и высоту; loadingLabel служит доступным именем. Спиннер IconButton также вращается. При prefers-reduced-motion анимация отключается. Это уточнение пользователя; Figma в этой итерации не изменялась.

Повторные проверки: 54 unit-теста и 100% четырёх метрик покрытия; 36 тестов каталога, включая регрессии ширины, центрирования, сохранения размера и вращения/reduced motion. Check и обе сборки прошли. Frontend/UI review выполнено; результат передан на повторное ревью PR #6.

...
# -*- coding: utf-8 -*-
"""
UI/UX Design Search - BM25 search engine for UI/UX guidance
...
def format_output(result, full=False):
    """Format token-efficient search results for an agent."""
    if "error" in result:
        return f"Error: {result['error']}"

    output = []
    if result.get("stack"):
        output.append("## UI/UX Design Stack Guidelines")
        output.append(f"**Stack:** {result['stack']} | **Query:** {result['query']}")
    else:
        output.append("## UI/UX Design Search Results")

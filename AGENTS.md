...
9. До начала прочитать `techDocs/README.md` и журнал согласования. Пользователь разрешил ранний технический frontend-блок по `techDocs/services/frontend/bootstrap-tasks.md`: каркас, сборка/запуск и UI-библиотека. Соблюдать согласования между блоками A–E. Это исключение не разрешает backend, БД, production-деплой или все продуктовые функции; остальные этапы сохраняются.

## Обязательный порядок выполнения задач с кодом

Покрытие тестами всех сервисов и модулей стремится к **100% собственного кода** (lines/statements/functions/branches); ≥90% — минимальный допуск, не целевой результат. Непокрытые сценарии явно обосновывать и фиксировать; достигнутые пороги не снижать ради прохождения проверок. Тестировать поведение, ошибки и границы, не подменять тесты формальным выполнением строк. Подробные правила и допустимые исключения: [тестирование](techDocs/process/testing.md).
